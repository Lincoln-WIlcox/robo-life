...
func _on_map_ui_power_pole_selected(power_pole: SelectableMapEntity) -> void:
	power_connector_selected.emit(power_pole.power_connector)

func _on_power_pole_map_entity_source_removed(removed_map_entity: SelectableMapEntity) -> void:
	...
	var iterating_map_entities: Array[MapEntity] = _map_data.get_map_entities().filter(func(map_entity: MapEntity): return map_entity is SelectableMapEntity).duplicate()
	for map_entity: SelectableMapEntity in iterating_map_entities:
